...
if($prevsub == 0) {
	...
	if($alternatesub == 1) {
		...
		$query = "DELETE FROM tagrelrecord WHERE userid = $userid AND urlid = $urlid AND tagid = $tagid AND tagresp = 1";
		mysqli_query($dbc, $query);
	}

	$query = "INSERT INTO tagrelrecord(userid, urlid, tagid, tagresp) VALUES($userid, $urlid, $tagid, 0)";
	mysqli_query($dbc, $query)

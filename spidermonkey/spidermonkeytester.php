...
echo '<br /> Random Test <br />';
while($row = mysqli_fetch_array($result)) {
	echo $row['rand_id'] . "\t";
	if($row['rand_urltitle'] != "") {
		echo '<a href = ' . $row['rand_url'] . ' target = "_blank">' . $row['rand_urltitle'] . '</a>' . ' from ' . $row['rand_url'];
	}
	else {
		echo '<a href = ' . $row['rand_url'] . ' target = "_blank">' . $row['rand_url'] . '</a>' . ' from ' . $row['rand_url'];
	}
	echo '<br />';
}

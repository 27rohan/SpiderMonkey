...
<title>RicoMonkey Most Popular</title>
</head>
...
foreach($cats as $cid)	{
  ...
  if($mostprefcat!='' && $cid = $mostprefcat) {
    $query = "SELECT * FROM urltable WHERE urlcat = '$cid' ORDER BY urlrank DESC LIMIT $doublepages";
    $result = mysqli_query($dbc, $query)
      or die('Error selecting from urltable table.');
  }

  else {
	  $query = "SELECT * FROM urltable WHERE urlcat = '$cid' ORDER BY urlrank DESC LIMIT $noofpages";
    $result = mysqli_query($dbc, $query)
		  or die('Error selecting from urltable table.');
  }
